...
export const NavigationCenter: React.FC = () => {
  ...
  const [mapKey, setMapKey] = React.useState(`map-nav-${Date.now()}`);
  ...
  React.useEffect(() => {
    ...
    return () => {
      setMapMounted(false);
      setMapKey(`map-nav-${Date.now()}-${Math.random()}`);
    };
  }, []);
    ...
    <div className="space-y-6 font-sans">
      ...
      <div className="flex justify-between items-center premium-card p-4.5">
        <div>
          ...
          <p className="text-xs text-gray-400 dark:text-gray-500 font-semibold">Shuttle dispatch timers, parking inventory tracking, and walking routes</p>
        </div>
      </div>
        ...
        <div className="lg:col-span-2 premium-card p-4.5 space-y-4">
          <div className="flex justify-between items-center">
            <div>
              ...
              <p className="text-[10px] text-gray-405 dark:text-gray-500 font-semibold">Real-time coordinates of transit hubs and stadium routes</p>
            </div>
          ...
          <div ref={mapRef} className="h-96 rounded-xl overflow-hidden relative border border-gray-150 dark:border-graphite-800">
            {mapMounted && (
              <MapContainer key={mapKey} center={[25.4208, 51.4886]} zoom={16} scrollWheelZoom={false}>
                <TileLayer
                ...
                {mapPoints.map((pt, idx) => (
                  <CircleMarker
                    ...
                    center={pt.position}
                    radius={10}
                    pathOptions={{ color: pt.color, fillColor: pt.color, fillOpacity: 0.8 }}
                  >
                    <Popup>
        ...
        <div className="space-y-6">
          ...
          <div className="premium-card p-4.5 space-y-4">
            <div className="flex items-center space-x-2">
              <Train className="w-4.5 h-4.5 text-forest-500" />
            ...
            <div className="space-y-2">
              {transitSchedules.map((tr, idx) => (
                <div key={idx} className="flex justify-between items-center text-xs p-3 border border-gray-150/15 dark:border-graphite-850 hover:bg-gray-55/50 dark:hover:bg-graphite-855/50 rounded-xl">
                  <div>
                    <span className="font-bold text-gray-700 dark:text-gray-305 block">{tr.line}</span>
                    <span className="text-[9px] text-gray-400 font-semibold">Destination: {tr.target}</span>
                  </div>
                  <div className="text-right">
                    <span className="text-xs font-bold text-gray-805 dark:text-gray-250 block">every {tr.frequency}</span>
                    <span className={`text-[8.5px] font-black uppercase ${
                      tr.status === 'ON_TIME' ? 'text-emerald-600' : 'text-amber-500'
                    }`}>
                      {tr.status.replace('_', ' ')}
                    </span>
                  </div>
          ...
          <div className="premium-card p-4.5 space-y-4">
            <div className="flex items-center space-x-2">
              <Bus className="w-4.5 h-4.5 text-forest-500" />
          ...
          <table className="w-full text-left text-xs text-gray-500 dark:text-gray-400">
            <thead>
              <tr className="border-b border-gray-150 dark:border-graphite-800 text-[10px] uppercase font-bold text-gray-400 pb-1.5">
                ...
                <th className="py-2">Destination Gate</th>
                <th className="py-2">Average Transit Time</th>
                <th className="py-2">Accessibility Path</th>
                <th className="py-2">Traffic Density</th>
              </tr>
            </thead>
            <tbody>
              {routesList.map((route) => (
                <tr key={route.id} className="border-b border-gray-150/20 dark:border-graphite-850/20 hover:bg-gray-50/50 dark:hover:bg-graphite-850/50">
                  <td className="py-2.5 font-semibold text-gray-800 dark:text-gray-300">{route.from}</td>
                  <td className="py-2.5 text-gray-600 dark:text-gray-405 font-semibold">{route.to}</td>
                  <td className="py-2.5 font-extrabold text-gray-700 dark:text-gray-200">{route.timeMin} mins</td>
                  <td className="py-2.5 font-semibold text-gray-400">{route.mode}</td>
                  <td className="py-2.5">
